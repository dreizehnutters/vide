...
if [[ "$PORT" = 1433 ]]; then
     sudo $NMAP $IP -Pn -n --open -p1433 --script-timeout 20s \
          UNKOWN="false"--script=ms-sql-con
     fig,ms-sql-dac,ms-sql-dump-hashes,ms-sql-empty-password,ms-sql-info,ms-sql-ntlm-info -g $PORT \
     --scan-delay $DELAY -oA $OUT_PATH
     UNKOWN="false"
fi
...
if [[ "$PORT" = 9100 ]]; then
     sudo $NMAP $IP -Pn -n --open -p9100 --script-timeout 20s \
          UNKOWN="false"--script=lexmark-con
     fig -g $PORT \
     --scan-delay $DELAY -oA $OUT_PATH
     UNKOWN="false"
fi
...
if [[ "$PORT" = 60010 || "$PORT" = 60030 ]]; then
     ...
     if [ -z "$UNKOWN" ]; then printf "\t${QP}no rule for port: $BD$PORT$RST\n"; 
fi
